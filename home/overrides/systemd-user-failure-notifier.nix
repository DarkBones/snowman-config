{ lib, pkgs, config, ... }:
let
  notifyFailedUserServices =
    pkgs.writeShellScriptBin "snowman-systemd-user-failure-notifier" ''
      set -euo pipefail
    ...
    lib.mkIf (pkgs.stdenv.isLinux && (config.roles.hyprland.enable or false)) {
      ...
      systemd.user.services.snowman-systemd-user-failure-notifier = {
        ...
        Service = {
          Type = "oneshot";
          ExecStart =
            "${notifyFailedUserServices}/bin/snowman-systemd-user-failure-notifier";
        };
      };
