...
let
  ...
  snowmanDotfilesScript = pkgs.writeShellScriptBin "snowman-dotfiles" ''
    set -euo pipefail
...
in {
  home.packages = [
    snowmanDotfilesScript
  ];

  home.file."bin/snowman-dotfiles".source =
    "${snowmanDotfilesScript}/bin/snowman-dotfiles";
}
